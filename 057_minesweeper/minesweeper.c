...
board_t * makeBoard(int w, int h, int numMines) {
  ...
  //Initialize all squares to UNKNOWN
  for (int y = 0; y < h; y++) {
    for (int x = 0; x < w; x++) {
      b->board[y][x] = UNKNOWN;
    }
  ...
  assert(numMines >= 0);
  //Randomly place mines on theboard
...
int countMines(board_t * b, int x, int y) {
  ...
  // Loop through the 8 neighboring squares
  for (int ny = y - 1; ny <= y + 1; ny++) {
    for (int nx = x - 1; nx <= x + 1; nx++) {
      // Skip the center square (x, y) itself
      if (nx == x && ny == y) {
        continue;
      }
      ...
      if (nx >= 0 && nx < b->width && ny >= 0 && ny < b->height) {
        // Use the IS_MINE macro to check for a mine
