...
class AVLMultiMap {
 private:
  ...
  void findBestFitHelper(Node * curr,
                         const K & cap_needed,
                         V *& best_fit,
                         K & best_key) const {
    if (curr == NULL) {
    ...
    if (curr->key == cap_needed) {
      best_fit = const_cast<V *>(&(*curr->vals.begin()));
      best_key = curr->key;
    ...
    if (curr->key >= cap_needed) {
      // Check current node
      for (typename std::set<V, CompareV>::iterator it = curr->vals.begin();
           it != curr->vals.end();
           ++it) {
        if (best_fit == NULL || cmp(curr->key, best_key) ||
            (!cmp(best_key, curr->key) && CompareV()(*it, *best_fit))) {
          best_fit = const_cast<V *>(&(*it));
          //best_fit = *it;
          best_key = curr->key;
        }
      }
      // Traverse left
      findBestFitHelper(curr->left, cap_needed, best_fit, best_key);
    }
    else {
      // Traverse right
      findBestFitHelper(curr->right, cap_needed, best_fit, best_key);
    }
  }
 ...
 public:
  ...
  V * findBestFit(const K & cap_needed) const {
    V * best_fit = NULL;
    K best_key = K();
    findBestFitHelper(root, cap_needed, best_fit, best_key);
    return best_fit ? best_fit : NULL;
  }
